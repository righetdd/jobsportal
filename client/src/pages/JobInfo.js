      ...
      <DefaultLayout>
        {job && (
          <div>
            ...
            <p><b>Deadline for applications</b> : {moment(job.applyUntil).format("DD.MM.YYYY HH:mm")}</p>
            <hr />
            <div className="flex justify-content-between">
            {job.postedBy == userid ? (<Button><Link to={`/editjob/${job._id}`}>Edit Now</Link></Button>) : alreadyApplied ? (<Tag color="green">Already Applied</Tag>) : (
                <Button onClick={applyNow}>Apply Now</Button>
              )}
              <p><b>Posted on</b> {moment(job.createdAt).format("DD/MM/YYYY")}</p>
            </div>
          </div>
        )}
      </DefaultLayout>
